...
function GarmentExchange() {
  ...
  const fetchExchangeRequests = async () => {
    try {
      ...
      console.log(fetchExchangeRequests)
    } catch (error) {
  ...
  const formatDate = (dateString) => {
    ...
    // Should not use timezone conversion if not handled correctly in backend and database
    ...
    <div>
              ...
              {member.garments && member.garments.length > 0 ? (
                member.garments.map((garment, index) => (
                  <div key={garment?._id || Math.random()}>
                    <Card
                      height="33em"
                      imgUrl={getImageFromURL(
                        garment?.fileFront || "fallbackImagePath",
                      )}
                      imgClassName={"container-card-img"}
                      title={
                        <label className="center text-purpleLight text-midLg">
                          {member.username}
                        </label>
                      }
                      status={
                        <label className="center text-purpleLight">
                          {index + 1}/{member.garments.length}
                        </label>
                      }
                      titleClassName={
                        "container-row clickable bg-purpleDark space-evenly"
                      }
                      description={
                        <div>
                      ...
                      footer={
                        member._id === user._id ? (
                          ...
                          <button
                            ...
                            onClick={() =>
                              openReservationModal(member._id, garment?._id)
                            }
                            disabled={
                              sentRequests[`${member._id}_${garment?._id}`]
                            }
                          >
                ...
                <Card
                  ...
                  imgClassName={"container-card-img"}
                  title={
                    <p className="center text-purpleLight text-midLg">
                      {member.username}
                    </p>
                  }
          ...
          <div className="container-grid-2-lg gap m2-v">
            {exchangeRequests.map((request) => (
              <div key={request._id} className="container-border clear-box">
                ...
                <p>
                  <strong>Exchange For:</strong>{" "}
                  {request.recipientGarmentDescription}
                </p>
                {request.recipientGarmentImage && (
                  <div className="container-input-img img-size-sm">
                    <img
                      src={getImageFromURL(request.recipientGarmentImage)}
                      alt="Recipient Garment"
                    />
                  </div>
                )}
                {request.status === "pending" &&
                  user._id === request.recipientId?._id && (
                    <div className="container-row space-evenly">
                      <button
                        className="button-accept"
                        onClick={() =>
                          handleExchangeResponse(request._id, "accepted")
                        }
                      >
                        Accept
                      </button>
                      <button
                        ...
                        onClick={() =>
                          handleExchangeResponse(request._id, "rejected")
                        }
                      >
                        Reject
                      </button>
      ...
      >
        ...
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          <br />
          <button className="button-regular full" onClick={handleModalSubmit}>
            Submit
          </button>
        </form>
      </Modal>
          ...
          {garmentModalID !== "" && (
            <div className="container">
              ...
              <div className="container-border page-tab">
                ...
                {tabPage === 2 && garmentModalID && (
                  <Composition garment={garmentModalID} />
                )}

                {tabPage === 3 && garmentModalID && (
