...
const GarmentDetails_p4 = ({
  ...
  const [nextMainKey, setNextMainKey] = useState(
    formData.compositionMain.length,
  )
  const [nextLiningKey, setNextLiningKey] = useState(
    formData.compositionLining.length,
  )
  const [nextPaddingKey, setNextPaddingKey] = useState(
    formData.compositionPadding.length,
  )
    ...
    switch (fieldType) {
      ...
      default:
        for (let i = 0; i < options.length; i++) {
          if (
            options[i].label.toLowerCase() === val ||
            val === 'material_other'
          ) {
            //remove error message if any
            addErrorMessage(e_to.id + '_error', null)
  ...
  function validatePage(type = '', index = 0) {
    ...
    if (type !== '') {
      ...
      for (let i = index; i < percentSet.length - 1; i++) {
        isValid =
          validate('percent', percentSet[i + 1].id, percentSet[i].id) && isValid
      }
    } else {
      let clothTypes = ['main']
  ...
  function isDuplicate(type, value) {
    ...
    switch (type) {
      case 'lining':
        values = formData.compositionLining.map((item) =>
          item.value.toLowerCase(),
        )
        break
      case 'padding':
        values = formData.compositionPadding.map((item) =>
          item.value.toLowerCase(),
        )
        break
      default:
        values = formData.compositionMain.map((item) =>
          item.value.toLowerCase(),
        )
    }
    ...
    <div>
      ...
      <div>
        ...
        {formData.compositionMain.map((inp, index) => {
          return (
            <div
              className="container-materials"
              key={'materials_main_' + index}
            >
              <label>{index + 1}</label>
              {nextMainKey > 1 && (
              ...
              <div className="container-materials-group">
                <div>
                  ...
                  >
                    ...
                    {options.map((opt) => (
                      <option key={'material_' + opt.value} value={opt.label}>
                        {opt.label}
                      </option>
                    ))}

                    <option value="material_other">Other</option>
                  </select>
                  {/* -------- */}
                  {/* <input
                    id={'main_mat_' + index}
        ...
        <div className="container-row">
          ...
          />
          <label className="clickable" onClick={() => addInput('main')}>
            Add Material
          </label>
        </div>
      </div>
        ...
        <div>
          <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (isDuplicate('lining', e.target.value)) {
                            addErrorMessage(
                              `lining_mat_${index}_error`,
                              'Material already selected',
                            )
                          } else {
                            formData.compositionLining[index].value =
                              e.target.value
                            setFormData({
                              ...formData,
                              compositionLining: [
                                ...formData.compositionLining,
                              ],
                            })
            ...
            <div className="container-row">
              ...
              />
              <label className="clickable" onClick={() => addInput('lining')}>
                Add Material
              </label>
            </div>
          </div>
        ...
        <div>
          <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (isDuplicate('padding', e.target.value)) {
                            addErrorMessage(
                              `padding_mat_${index}_error`,
                              'Material already selected',
                            )
                          } else {
                            formData.compositionPadding[index].value =
                              e.target.value
                            setFormData({
                              ...formData,
                              compositionPadding: [
                                ...formData.compositionPadding,
                              ],
                            })
            ...
            <div className="container-row">
              ...
              />
              <label className="clickable" onClick={() => addInput('padding')}>
                Add Material
              </label>
            </div>
          </div>
