...
const GARMENT_TYPES = [
  { cat: 'Top', value: 'tShirt', label: 'T-shirt (long or short sleeve)' },
  {
    cat: 'Top',
    value: 'poloShirt',
    label: 'Polo shirt (long or short sleeve)',
  },
  { cat: 'Top', value: 'sweatshirt', label: 'Sweatshirt or Hoodie' },
  {
    ...
    value: 'sweater',
    label:
      'Sweater or Pullover (usually made of knitted or crocheted material)',
  },
  {
    cat: 'Top',
    value: 'buttonDownShirt',
    label: 'Button-down shirt (long or short sleeve)',
  },
  { cat: 'Top', value: 'blouse', label: 'Blouse (long or short sleeve)' },
  {
    cat: 'Top',
    value: 'waistcoat',
    label: 'Waistcoat or Vest (sleeveless upper-body garment)',
  },
  ...
  },
  {
    ...
    value: 'suitJacket',
    label:
      'Suit Jacket (also called a lounge jacket, classic menswear as part of a suit)',
  },
  {
    cat: 'Top',
    value: 'jacket',
    label:
      'Jacket (for the upper body, usually extending below the hips, usually for outerwear)',
  },
  {
    cat: 'Top',
    value: 'coat',
    label:
      'Coat (longer than a jacket, usually outerwear, for example, a trench coat)',
  },
  { cat: 'Top', value: 'rainJacket', label: 'Rain Jacket' },
  {
    cat: 'Bottom',
    value: 'pants',
    label: 'Pants (more casual can vary in length, for example, cargo pants)',
  },
  {
    cat: 'Bottom',
    value: 'trouser',
    label:
      'Trouser (more formal and covers the entire lower body from the waist to the ankles)',
  },
...
const CARE_WASH_METHODS = {
  ...
  Heat: [
    {
      value: 'washHeatXXC',
      label: 'Input temperature (' + String.fromCharCode(176) + 'C)',
    },
    {
      value: 'washHeatXXF',
      label: 'Input temperature (' + String.fromCharCode(176) + 'F)',
    },
  ],
  Temp: [],
}
...
for (let key in careInstructions) {
  if (
    careInstructions[key].type === 'wash' &&
    careInstructions[key].subtype !== ''
  ) {
    CARE_WASH_METHODS[careInstructions[key].subtype].push({
      value: careInstructions[key].value,
      label: careInstructions[key].name,
    })
  } else if (
    careInstructions[key].type === 'dry' &&
    careInstructions[key].subtype !== ''
  ) {
    ...
    careInstructions[key].type === 'tumble' &&
    careInstructions[key].subtype !== ''
  ) {
    CARE_DRY_METHODS[careInstructions[key].subtype].push({
      value: careInstructions[key].value,
    ...
    careInstructions[key].type === 'dryC' &&
    careInstructions[key].subtype !== ''
  ) {
    ...
    careInstructions[key].type === 'iron' &&
    careInstructions[key].subtype !== ''
  ) {
    ...
    careInstructions[key].type === 'bleach' &&
    careInstructions[key].subtype !== ''
  ) {
...
const PICKUP_LOCS = [
  {
    ...
    label: 'Outside the Newnham building B fashion boutique',
  },
...
const PICKUP_CAMPUS = [
  ...
  { value: 'Peterbrorough', label: 'Peterbrorough Campus' },
